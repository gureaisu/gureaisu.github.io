...
### 2.2 個人報表（`WriteMemberReport` → `MemberReportAdd`）
...
### 3.2 個人報表（`WriteMemberReport` → `MemberReportAdd`）
...
| Key | 說明 |
|-----|------|
| `RedisHost` | Redis 主機位址 |
| `RedisPort` | 預設 6379 |
| `RedisPassword` | 密碼 |

---

## 7. 驗證 Checklist（程式改動後才需要執行）

1. 確認 `WriteInningReport` / `WriteMemberReport` 走到 MySQL SP（而不是原本的 XPG API）
2. 觸發至少一次：
...
3. 特別驗證 Rocket 的 `p_Odds`：
   - `Odds` 是否正確落到 `T_GameRound.Odds` 與 `T_Bet.Odds`
4. 特別驗證 GuessSong 的補參數行為：
   - `p_Odds` / `p_Rake` 指定 `0` 是否符合 DB 約束
