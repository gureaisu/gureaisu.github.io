# GameServer 存取 DB（只做報表寫入）—SP 參數對照文件

本文目的：在後續把「報表寫入」改成直連 MySQL 執行預存程序時，能快速定位：

- GameServer 目前「局報表 / 個人報表」實際寫入位置在哪裡
- 現行程式送出的欄位長什麼樣
- 你提供的兩個 SP（`sp_Game_GameRoundReportAdd`、`sp_Game_MemberReportAdd`）如何逐參數對應到現有 report 欄位
...
- `d:/WorkDir/GPG/gs-rocket/code/GameServer/GameController.BgMain.cs`
...
- `GameController.BgMain.cs` 裡對 `ManagerDB.Inst.WriteInningReport(...)` 與 `ManagerDB.Inst.WriteMemberReport(...)` 是註解掉的（目前報表寫入走 Adapter 的 override，而不是走 `IDbProvider`）。

### 1.2 實際委派給哪個 Adapter

報表由 `ContentLobbyAdapter` 子類別（各遊戲的 LobbyAdapter.Report.cs）負責實作：

- GuessSong：`d:/WorkDir/GPG/gs-rocket/code/GameServer/Content/GuessSong/GuessSongLobbyAdapter.Report.cs`
- Rocket：`d:/WorkDir/GPG/gs-rocket/code/GameServer/Content/Rocket/RocketLobbyAdapter.Report.cs`
...
## 2. GuessSong：目前「局/個人報表」送出的欄位（用於 SP 對應）

檔案：

- `d:/WorkDir/GPG/gs-rocket/code/GameServer/Content/GuessSong/GuessSongLobbyAdapter.Report.cs`

### 2.1 GuessSong 局報表（`WriteInningReport` → `GameRoundReportAdd`）

`writeInningReport_ByXpgApi(...)` 組合 JSON 時目前送出包含：
...
你提供的 SP `sp_Game_GameRoundReportAdd` 參數中：

- `p_Odds` 需要 `decimal(10,4)`；GuessSong 目前沒有對應欄位 → 實作 SP 呼叫時可用 `0`（或依你 DBA 的規則用預設值）。

另外：SP 需要 `p_Created`，但現行 JSON 不包含 `Created` → 實作 SP 呼叫時需要補 `p_Created`（建議用 `DateTimeOffset.Now`，或與 `SettleTime/EndTime` 使用同一個時間點）。

### 2.2 GuessSong 個人報表（`WriteMemberReport` → `MemberReportAdd`）

`writeMemberReport_ByXpgApi(...)` 組合 JSON 時目前送出包含：

- `GameId`：`this.GameID`（字串）
- `MemberId`：`report.MemberID`
- `Account`：`paras.Account`（`report.PlayerParas` 轉型為 `DBXpgApi.XpgSystemParas`）
- `CompanyId`：`paras.CompanyId`
- `StreamChannelId`：`report.LoginSourceID`（目前用登入來源當作 StreamChannelId）
...
SP `sp_Game_MemberReportAdd` 參數中：

- `p_Odds` 需要 `decimal(10,4)`；GuessSong 目前沒有對應欄位 → 實作 SP 呼叫時可用 `0`。
- SP 需要 `p_Created`、`p_Odds`，現行 JSON 不包含 `Created`、`Odds` → 實作 SP 呼叫時需要補。

---

## 3. Rocket：目前「局/個人報表」送出的欄位（用於 SP 對應）

檔案：

- `d:/WorkDir/GPG/gs-rocket/code/GameServer/Content/Rocket/RocketLobbyAdapter.Report.cs`

### 3.1 Rocket 局報表（`WriteInningReport` → `GameRoundReportAdd`）
...
- `Odds`：`report.InningOdds`（這裡有 Odds）

SP `sp_Game_GameRoundReportAdd`：

- `p_Odds` 可以直接對應 `Odds`。
- `p_Created` 現行 JSON 不包含 → 需要補（建議 `DateTimeOffset.Now`）。

### 3.2 Rocket 個人報表（`WriteMemberReport` → `MemberReportAdd`）
...
- `BetTime`：`report.InningTime`
- `SettleTime`：`DateTimeOffset.Now`
- `GSRoundId`：`String.Format("{0}_{1}", report.SequenceID, report.InningID)`（現行為 `$"{...}_{...}"`）
- `Odds`：`report.PlayerOdds`

SP `sp_Game_MemberReportAdd`：
...
對照表（適用於 GuessSong / Rocket，差異會標註）：

| SP 參數 | 對應來源（現行程式） | 備註 |
|---|---|---|
| `p_GameId` | `this.GameID`（需 `int.Parse(...)`） | 現行 `GameID` 是字串 |
| `p_StartTime` | `report.InningTime` | datetime(6) 精度需由 DB driver 轉換/截斷 |
| `p_EndTime` | `DateTimeOffset.Now` | 現行 JSON 沒有 `Created`，但有 `EndTime` |
...
| `p_GSRoundId` | `"{SequenceID}_{InningID}"` | 現行：`$"{report.SequenceID}_{report.InningID}"` 或 `String.Format("{0}_{1}",...)` |
| `p_GSRoundSetId` | `report.SequenceID` |  |
| `p_Odds` | Rocket：`report.InningOdds`；GuessSong：`0` | GuessSong 目前沒有 odds 欄位 |

補充說明：

- 現行 GameServer JSON 在「局報表」送出的是 `GameRoundReportAdd`，而 SP 會再 INSERT/UPDATE（包含 `UPDATE T_Bet SET GameRoundId = v_GameRoundId WHERE GSRoundId = p_GSRoundId;`）。
- 因此實作 MySQL SP 時，`GameRoundReportAdd` 與 `MemberReportAdd` 的執行順序要確認：通常先有會員下注記錄（T_Bet 未綁 GameRoundId）再寫局報表（補上 GameRoundId）。
...
| `p_Currency` | `paras.Currency` | 你已確認等於 `T_Currency.Code` |
| `p_GameId` | `this.GameID`（需 `int.Parse(...)`） | 現行 `GameID` 是字串 |
...
| `p_Rake` | GuessSong：`report.Rake`（若存在）；Rocket：`report.Rake` | 若 GuessSong 的 member report DTO 目前沒有 `Rake` 欄位，需確認：目前 GuessSong member JSON 沒有送 `Rake`（但 SP 要 `p_Rake`）。建議實作時指定 `0` 或從其它欄位計算。 |
| `p_Created` | （建議）`DateTimeOffset.Now` | 現行 JSON 不包含 `Created` |
...
- `sp_Game_MemberReportAdd` 需要 `p_Rake` 與 `p_Odds` 與 `p_Created`，但 GuessSong 現行 `writeMemberReport_ByXpgApi` 的 JSON 沒有送出 `Rake` 與 `Odds`，也沒有送出 `Created`。
- 這代表你後續實作直連 SP 時，需要補齊這些參數（通常設為 `0` 或按遊戲規則取得值）。
...
- MySQL port：`3306`
- `EXECUTE` 權限：已授權
- `paras.Currency` 的值與 DB 端 `T_Currency.Code`：一致

---

## 6. 直連 MySQL/Redis 時設定項建議（文件用，不含實作）

現行專案設定檔載入在 `d:/WorkDir/GPG/gs-rocket/code/GameServer/GameSetting.cs`，`_setting/_Config.json` 目前可用的欄位主要是：

- `DB_ConnString` / `Wallet_ConnString`：目前被 `DBXpgApi` 當作 REST API base URL 使用
- `SystemRunMode`：決定 `ManagerDB.Inst` 選哪個 `IDbProvider`

因為你說「先只做報表寫入」，通常你會在 Adapter 的 `WriteInningReport` / `WriteMemberReport` 改成直連 MySQL 執行 SP。
